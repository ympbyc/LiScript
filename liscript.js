LiScript = (function () {
    ...
    var translations = {
        fn: function (argList, body) {
            ...
            if (compiledBody.length > 1)
                return '(function('
                     + argList.join(',')
                     + ') { '
                     + compiledBody.slice(-1).join(";")
                     + '; return '
                     + compiledBody[compiledBody.length-1]
                     + '; })';
            return '(function('
                 + argList.join(',')
                 + ') { return '
        ...
        macro: function (argList, body) {
            return '{type:"macro", fn:'
                 + tree_to_js(["fn"].concat(argList,body))
                 + '}';
        },
        ...
        obj: function () {
            ...
            return '({'
                 + pairs.map(function (a) { return a[0]+':'+tree_to_js(a[1]); }).join(',')
                 + '})';
        },
        arr: function () {
            return '(['
        ...
        iter:function (obj, code) {
            return '(function (obj_) { for(var key in obj_) { var val = obj_[key];'
                 + tree_to_js(code)
                 + '}})('
                 + tree_to_js(obj)
                 + ')';
    ...
    var add_reader = function (head, open, close) {
        readers[open] = {close: close, head: head};
    };
        ...
        return (function parse_object(close){
            ...
            function matchingReader(str){
                for (var reader in readers)
                    if (str.slice(0,reader.length)===reader)
                        return { open:  reader,
                                 close: readers[reader].close,
                                 head:  readers[reader].head };
                return false;
            }
            for (; str.length > 0; str = str.slice(1)) {
                if (str[0] == close) break;
                    ...
                    obj.push((reader.head ? [reader.head] : [])
                       .concat(parse_object(reader.close)));
                }
    ...
    var tree_to_js = function(ast){
        return typeof ast !== 'object' ? ast
            ...
            :  '('
             + tree_to_js(ast[0])
             + '('
             + ast.slice(1).map(function (a) {return tree_to_js(a);}).join(',')
             + '))';
    };
